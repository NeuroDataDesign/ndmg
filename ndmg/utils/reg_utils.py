...
def t1w_skullstrip(t1w, out, skull="none"):
    ...
    """
    if skull == "below":
        cmd = "3dSkullStrip -prefix {} -input {} -shrink_fac_bot_lim 0.6 -ld 45".format(
            out, t1w
        )
    elif skull == "cerebelum":
        cmd = "3dSkullStrip -prefix {} -input {} -shrink_fac_bot_lim 0.3 -ld 45".format(
            out, t1w
        )
    elif skull == "eye":
...
def align(
    ...
    if cost == 'bbr':
        ...
        if img_inp_arr.ndim > 3:
            ...
            sam_1 = affine_map.transform(img_inp_arr)
            c_of_mass = transform_centers_of_mass(img_ref_arr, img_ref.affine, img_inp_arr, img_inp.affine)
            sam_2 = c_of_mass.transform(img_inp_arr)
            nbins = 32
            sampling_prop = None
